...
def data_loader(dimension, nBin, filter_var):
    ...
    # data = np.stack([data[:,:dimension-1],diff],axis=-1)
    data = np.stack([data[:,:dimension],tan],axis=-1)
    ...
    # diff = tan[:,:dimension-1] - tan[:,1:]
    # X_test = np.stack([X_test[:,:dimension-1],diff],axis=-1)
    X_test = np.stack([X_test[:,:dimension],tan],axis=-1)
...
class ResNet18(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.convA1 = tf.keras.layers.Conv1D(64,7,2,activation=tf.nn.relu,input_shape=(dimension,2),padding='same')
        ...
        self.normF = tf.keras.layers.MaxPooling1D(2,padding='same')
