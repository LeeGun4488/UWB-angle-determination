...
path = glob.glob(os.path.join('Datasets', '*'))
...
# lr_scheduler = LearningRateScheduler(step_decay, verbose=1)  , callbacks=[lr_scheduler]

model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate = 0.001),
              loss="categorical_crossentropy",
              metrics=['accuracy'])
history = model.fit(X_train, y_train, batch_size = 2000, epochs=100, validation_data = (X_val, y_val))
